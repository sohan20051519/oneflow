...
# ---------------------------------------------------------------------------
# Stub endpoints for routes that reference commercial-only views.
# These return minimal valid responses so the URL router can load without
# ImportError while keeping the community edition functional.
# ---------------------------------------------------------------------------

class WorkspacePreferencesEndpoint(BaseAPIView):
    """Workspace preferences (CE stub)."""
    def get(self, request, slug):
...
class WorkspaceProjectMembersLiteEndpoint(BaseAPIView):
    """Lite project members listing (CE stub — returns same as full endpoint)."""
    permission_classes = [WorkspaceEntityPermission]
...
class WorkspaceMemberUserOnboardingEndpoint(BaseAPIView):
    """User onboarding status (CE stub)."""
    def get(self, request, slug):
        workspace_member = WorkspaceMember.objects.filter(
            workspace__slug=slug, member=request.user, is_active=True
        ).first()
...
class UserWorkspaceProjectRolesEndpoint(BaseAPIView):
    """User project roles within workspace (CE stub)."""
    def get(self, request, slug):
        project_members = ProjectMember.objects.filter(
            workspace__slug=slug, member=request.user, is_active=True
        ...
        for pm in project_members:
            roles[str(pm.project_id)] = pm.role
