...
function StoreWrapper(props: TStoreWrapper) {
  ...
  useEffect(() => {
    ...
    // Only initialize theme on FIRST load for this user
    if (hasInitializedThemeRef.current) {
      return;
    }

    const localTheme = typeof window !== "undefined" ? localStorage.getItem("theme") : null;
    const serverTheme = userProfile?.theme?.theme;

    // Prioritize user's explicit local setting, then server profile, default to light
  ...
  }, [userProfile, setTheme, updateUserTheme]);
