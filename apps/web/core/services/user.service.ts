...
export class UserService extends APIService {
  ...
  async getUserProfileProjectsSegregation(
    ...
    const userPromise = this.get(`/api/workspaces/${workspaceSlug}/user-profile/${userId}/`)
      .then((response) => response?.data)
      .catch((error) => {
        throw error?.response?.data;
      });

    const projectStatsPromise = this.get(
      `/api/workspaces/${workspaceSlug}/user-profile/${userId}/project-stats/`
    )
      .then((response) => {
    ...
    const [userDataRes, projectStats] = await Promise.all([userPromise, projectStatsPromise]);

    const isWrapped = userDataRes && typeof userDataRes === "object" && "user_data" in userDataRes;
    const userData = isWrapped ? userDataRes.user_data : userDataRes;
    const projectData =
      isWrapped && Array.isArray(userDataRes.project_data)
        ? userDataRes.project_data
        : projectStats;
