...
export class WorkspaceService extends APIService {
  ...
  async workspaceMemberMe(workspaceSlug: string): Promise<IWorkspaceMemberMe> {
    return this.get(`/api/workspaces/${workspaceSlug}/workspace-members/me/`)
      ...
      .catch(async (error) => {
        try {
          const wsList = await this.userWorkspaces();
          const matchedWs = wsList?.find((w) => w.slug === workspaceSlug);
          if (matchedWs) {
            return {
              id: matchedWs.id,
              member: typeof matchedWs.owner === "string" ? matchedWs.owner : matchedWs.owner?.id,
              role: matchedWs.role ?? 20,
              workspace: matchedWs.id,
              company_role: null,
              created_at: matchedWs.created_at,
              created_by: matchedWs.created_by,
              updated_at: matchedWs.updated_at,
              updated_by: matchedWs.updated_by,
      ...
      .catch(async () => {
        return this.get(`/api/workspaces/${workspaceSlug}/project-members/`)
          .then((res) => {
            ...
            for (const [projectId, members] of Object.entries(data)) {
              if (Array.isArray(members) && members.length > 0) {
                roles[projectId] = (members[0] as any)?.role ?? 20;
              }
            }
            return roles;
          })
