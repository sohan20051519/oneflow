...
export const ModuleViewHeader = observer(function ModuleViewHeader() {
  ...
  return (
    <div className="hidden h-full items-center gap-2 self-end sm:flex">
      ...
      <div className="hidden h-8 items-center gap-0.5 rounded-lg border border-subtle bg-layer-1 p-0.5 md:flex">
        {MODULE_VIEW_LAYOUTS.map((layout) => (
          <Tooltip key={layout.key} tooltipContent={t(layout.i18n_title)} isMobile={isMobile}>
            <button
              type="button"
              className={cn(
                "group grid h-6.5 w-7 place-items-center overflow-hidden rounded-md transition-all hover:bg-layer-2",
                {
                  "bg-layer-2 shadow-xs text-primary":
