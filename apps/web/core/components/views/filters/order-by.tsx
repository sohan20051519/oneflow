...
export function ViewOrderByDropdown(props: Props) {
  ...
  const buttonClassName = isMobile
    ...
    : cn(getButtonStyling("secondary", "sm"), "px-2 text-tertiary");
    ...
    <CustomMenu
      customButton={
        <span className={buttonClassName}>
          {!isMobile && icon}
          <span className="shrink-0 hidden md:inline"> {orderByDetails?.i18n_label && t(orderByDetails?.i18n_label)}</span>
          <ChevronDownIcon className="size-3 hidden md:inline" strokeWidth={2} />
        </span>
      }
