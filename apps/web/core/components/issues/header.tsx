    ...
    <Header>
      <Header.LeftItem>
        <div className="flex min-w-0 items-center gap-2.5">
          <Breadcrumbs onBack={() => router.back()} isLoading={loader === "init-loader"} className="min-w-0 flex-shrink truncate">
        ...
        {currentProjectDetails?.anchor ? (
          <a
            href={publishedURL}
            className="group hidden sm:flex shrink-0 items-center gap-1.5 rounded-sm bg-accent-primary/10 px-2.5 py-1 text-11 font-medium text-accent-primary"
            target="_blank"
      ...
      <Header.RightItem>
        ...
        {canUserCreateIssue && (
          ...
          >
            ...
            <span className="hidden xl:inline">{t("issue.add.label")}</span>
            <span className="hidden sm:inline xl:hidden">{t("issue.label", { count: 1 })}</span>
          </Button>
        )}
      </Header.RightItem>
    </Header>
