...
export function ProfileStateDistribution({ stateDistribution, userProfile }: Props) {
  ...
  const distributionList = Array.isArray(userProfile.state_distribution) ? userProfile.state_distribution : [];
        ...
        {distributionList.length > 0 ? (
          <div className="grid h-[300px] w-full grid-cols-1 gap-x-6 md:grid-cols-2">
            <PieChart
              ...
              data={
                distributionList.map((group) => ({
                  id: group.state_group,
                  key: group.state_group,
                  value: group.state_count,
                  name: capitalizeFirstLetter(group.state_group ?? ""),
                  color: STATE_GROUPS[group.state_group]?.color ?? "var(--background-color-accent-primary)",
                })) ?? []
              }
              cells={distributionList.map((group) => ({
                key: group.state_group,
                ...
                {(stateDistribution ?? []).map((group) => (
                  <div key={group.state_group} className="flex items-center justify-between gap-2 text-11">
                    <div className="flex items-center gap-1.5">
                      ...
                      />
                      <div className="whitespace-nowrap">{STATE_GROUPS[group.state_group]?.label ?? group.state_group}</div>
                    </div>
