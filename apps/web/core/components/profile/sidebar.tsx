...
export const ProfileSidebar = observer(function ProfileSidebar(props: TProfileSidebar) {
  ...
  // derived values
  const userData = (userProjectsData as any)?.user_data ?? userProjectsData;
  ...
  return (
    <div
      ...
      style={profileSidebarCollapsed && typeof window !== "undefined" ? { marginLeft: `${window.innerWidth || 0}px` } : {}}
    >
      {userProjectsData ? (
        <>
          ...
          <div className="px-5">
            ...
            <div className="mt-9 divide-y divide-subtle">
              {(Array.isArray(userProjectsData?.project_data) ? userProjectsData.project_data : []).map((project, index) => {
                const projectDetails = getProjectById(project.id);

                const created_issues = project.created_issues || 0;
                const assigned_issues = project.assigned_issues || 0;
                const pending_issues = project.pending_issues || 0;
                const completed_issues = project.completed_issues || 0;
                const totalIssues = created_issues + assigned_issues + pending_issues + completed_issues;

                const completedIssuePercentage =
                  assigned_issues === 0
                    ? 0
                    : Math.round((completed_issues / assigned_issues) * 100);
