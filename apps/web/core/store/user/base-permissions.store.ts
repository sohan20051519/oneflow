...
export class BaseUserPermissionStore implements IBaseUserPermissionStore {
  ...
  workspaceInfoBySlug = computedFn((workspaceSlug: string): IWorkspaceMemberMe | undefined => {
    ...
    if (this.workspaceUserInfo[workspaceSlug]) {
      return this.workspaceUserInfo[workspaceSlug];
    }
    const ws = this.store.workspaceRoot.getWorkspaceBySlug(workspaceSlug);
    if (ws) {
      return {
        id: ws.id,
        member: typeof ws.owner === "string" ? ws.owner : ws.owner?.id,
        role: ws.role ?? 20,
        workspace: ws.id,
        company_role: null,
  ...
  fetchUserWorkspaceInfo = async (workspaceSlug: string): Promise<IWorkspaceMemberMe> => {
    try {
      ...
      const ws = this.store.workspaceRoot.getWorkspaceBySlug(workspaceSlug);
      if (ws) {
        const fallbackInfo: IWorkspaceMemberMe = {
          id: ws.id,
          member: typeof ws.owner === "string" ? ws.owner : ws.owner?.id,
          role: ws.role ?? 20,
          workspace: ws.id,
        ...
        runInAction(() => {
          set(this.workspaceUserInfo, [workspaceSlug], fallbackInfo);
          this.loader = false;
        });
        return fallbackInfo;
      }
