...
export class CycleStore implements ICycleStore {
  ...
  fetchWorkspaceCycles = async (workspaceSlug: string) =>
    await this.cycleService.getWorkspaceCycles(workspaceSlug).then((response: any) => {
      runInAction(() => {
  ...
  fetchAllCycles = async (workspaceSlug: string, projectId: string) => {
    try {
      this.loader = true;
      await this.cycleService.getCyclesWithParams(workspaceSlug, projectId).then((response: any) => {
        runInAction(() => {
          const list = Array.isArray(response) ? response : (response?.results ?? []);
          list.forEach((cycle: any) => {
  ...
  fetchActiveCycle = async (workspaceSlug: string, projectId: string) =>
    await this.cycleService.getCyclesWithParams(workspaceSlug, projectId, "current").then((response: any) => {
      runInAction(() => {
        const list = Array.isArray(response) ? response : (response?.results ?? []);
        list.forEach((cycle: any) => {
          set(this.activeCycleIdMap, [cycle.id], true);
