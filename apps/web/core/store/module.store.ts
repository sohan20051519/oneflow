...
export class ModulesStore implements IModuleStore {
  ...
  fetchWorkspaceModules = async (workspaceSlug: string) =>
    await this.moduleService.getWorkspaceModules(workspaceSlug).then((response: any) => {
      runInAction(() => {
        ...
        const uniqueProjectIds = new Set(list.map((module: any) => module.project_id));
        uniqueProjectIds.forEach((projectId: any) => {
  ...
  fetchModules = async (workspaceSlug: string, projectId: string) => {
    try {
      this.loader = true;
      await this.moduleService.getModules(workspaceSlug, projectId).then((response: any) => {
        runInAction(() => {
          const list = Array.isArray(response) ? response : (response?.results ?? []);
          list.forEach((module: any) => {
            set(this.moduleMap, [module.id], { ...this.moduleMap[module.id], ...module });
  ...
  fetchModulesSlim = async (workspaceSlug: string, projectId: string) => {
    try {
      ...
      await this.moduleService.getWorkspaceModules(workspaceSlug).then((response: any) => {
        const list = Array.isArray(response) ? response : (response?.results ?? []);
        const projectModules = list.filter((module: any) => module.project_id === projectId);
        runInAction(() => {
